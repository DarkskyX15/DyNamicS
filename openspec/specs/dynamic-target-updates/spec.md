# dynamic-target-updates Specification

## Purpose
定义基于更新令牌的动态目标更新能力，包括鉴权、模式校验、目标值变更与审计记录要求，并约束不同 target 更新模式下允许的载荷类型。

## Requirements

### Requirement: 基于令牌鉴权的 target 更新
系统 MUST 提供一个使用 target 级更新令牌进行鉴权的目标更新 API。
...
系统 MUST 支持对处于动态主机模式的 `target` 执行仅主机值更新。
...
系统 MUST 支持对处于动态 URL 模式的 `target` 执行完整 URL 更新。
