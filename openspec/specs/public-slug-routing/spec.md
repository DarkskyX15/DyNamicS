# public-slug-routing Specification

## Purpose
定义公开 slug 跳转与公开信息查询能力，包括跳转响应、query 合并、公开信息暴露与目标不可用时的安全失败处理。

## Requirements

### Requirement: 基于 slug 的公开跳转
系统 MUST 提供一个公开端点，用于将全局唯一的 `slug` 解析到其当前目标，并返回面向浏览器使用的 HTTP 跳转响应。
...
系统 MUST 允许每个 `slug` 从受限的浏览器友好跳转状态码集合中保存自己的跳转码。
...
系统 MUST 在构造跳转目标地址时保留传入请求的 query string。
...
系统 MUST 提供一个公开端点，用于返回某个 `slug` 当前可公开的目标信息。
...
系统 MUST 避免将请求跳转到一个已禁用或无法解析为合法目标 URL 的 `target`。
