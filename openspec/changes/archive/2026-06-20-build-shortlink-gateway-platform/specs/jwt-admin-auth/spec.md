...
系统必须使用“JWT access token + opaque refresh token”的登录流程对管理用户进行鉴权。
...
系统必须要求所有受保护的管理 API 携带有效的 access token。
...
系统必须提供 refresh 流程，使管理客户端在 refresh token 仍然有效时，无需重新登录即可获取新的 access token。
...
系统必须将 refresh token 视为服务器签发的随机 opaque token，并仅在服务端保存其哈希值和会话状态。
