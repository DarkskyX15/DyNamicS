...
系统必须提供带鉴权的管理 API，使用户在没有管理员权限时只能管理自己的 `slug`、`target`、更新令牌和日志。
...
系统必须允许一个用户创建多个 `slug`，并允许多个 `slug` 绑定到同一个 `target`。
...
系统必须支持静态 `target`、主机动态更新 `target` 和完整 URL 动态更新 `target`。
...
系统必须允许用户为自己拥有的 `target` 创建、禁用和撤销更新令牌。
...
### Requirement: 更新审计可见性
系统必须记录并通过管理 API 暴露 `target` 的更新历史。
